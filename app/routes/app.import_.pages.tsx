...
import { saveImportReport } from "../migrate/import.server";
import {
  buildPagesPlan,
  executePagesBatch,
  parsePagesFile,
} from "../migrate/pages-import.server";
...
export const action = async ({ request }: ActionFunctionArgs): Promise<ActionData> => {
  ...
  let file;
  try {
    file = parsePagesFile(await uploaded.text());
  } catch (error) {
    return {
      mode: "error",
  ...
  if (intent === "validate") {
    const plan = await buildPagesPlan(admin.graphql, file);
  ...
  if (intent === "batch") {
    ...
    const { results, pending } = await executePagesBatch(admin.graphql, file, { exclude });
    ...
    >
      <thead>
        <tr style={{ textAlign: "left", borderBottom: "1px solid #e1e3e5" }}>
          <th style={{ padding: "8px" }}>页面 (handle)</th>
...
export default function ImportPagesPage() {
  ...
  const startImport = () => {
    ...
    setProgress({
      processed: 0,
      pending: plan ? plan.summary.create : 0,
    });
    submitWithFile("batch");
  };
  ...
  return (
    <s-page heading="导入页面">
      <s-section heading="上传页面迁移文件">
        <s-paragraph>
          在目标店铺使用本页面。上传由源店铺导出的页面 JSON
          文件，先执行预检（不会写入任何数据），确认无误后再正式导入。
          目标店铺已存在相同 handle 的页面将被跳过，不会覆盖。
        </s-paragraph>
      ...
      {planData && !report && (
        <>
          <s-section
            heading={`预检结果：创建 ${planData.plan.summary.create} / 跳过 ${planData.plan.summary.skip}`}
          >
            <ItemsTable items={planData.plan.items} />
          </s-section>
          {planData.skipped.length > 0 && (
            <s-section heading={`导出时已跳过的项目（${planData.skipped.length}）`}>
      ...
      {report && (
        <s-section
          heading={`导入完成：创建 ${report.summary.create} / 跳过 ${report.summary.skip} / 失败 ${report.summary.fail}`}
        >
          {report.summary.fail > 0 ? (
            <s-banner tone="critical">
              有 {report.summary.fail} 页导入失败，请查看下表原因。本次记录已保存到「迁移历史」。
            </s-banner>
          ) : (
            <s-banner tone="success">全部页面导入成功，记录已保存到「迁移历史」。</s-banner>
          )}
