...
This app does not use a database. Session data and migration history are stored via `app/storage.server.ts`:

- When deployed on **Netlify**, [Netlify Blobs](https://docs.netlify.com/build/data-and-storage/netlify-blobs/) is used (two stores: `shopify-sessions` and `migration-runs`).
- In **local development**, in-memory storage is used (data does not persist across restarts).
